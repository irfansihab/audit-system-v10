...
- **Tool inti:** `read_context` → `read_ingested_digest`/`search_bukti` → penilaian per komponen/sub-unsur SPIP → `append_temuan` (catatan/AoI **tanpa unsur Sebab** — evaluasi ber-LKE, bukan KKSA) → `record_pkp_assessment` → `render_kkp_docx` → `run_qc_kkp`.
...
3. **Catat penilaian per sub-unsur** lewat `record_pkp_assessment` dan temuan/AoI lewat `append_temuan` (**tanpa unsur Sebab** — evaluasi ber-LKE, bukan KKSA) — hindari menulis ulang file JSON secara manual.
