...
>   5. Setelah SEMUA unsur dinilai, susun **catatan/AoI via `append_temuan` — TANPA unsur Sebab** (evaluasi ber-LKE, bukan KKSA; isi Kondisi/Kriteria/Akibat + sumber) dari selisih PM vs APIP → `render_kkp_docx` (KKP memuat rekap predikat/skor LKE + AoI) → `run_qc_kkp`.
>   Urutan wajib: `read_lke` → nilai APIP semua unsur → `fill_lke` → bandingkan PM vs APIP → catatan/AoI (tanpa Sebab). **Jangan** panggil `record_pkp_assessment` (bukan tool).
