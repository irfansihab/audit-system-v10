...
>   - **⚠ PENGECUALIAN untuk RKA-K/L & PENGADAAN (multi-unit):** unit objek (RKA: RO; pengadaan: paket) bisa ditambah kapan saja. **RKA:** SELALU mulai REFINE dengan `run_batch_rka` → `read_digest` (index per RO; `n_temuan` per RO → RO baru = `n_temuan=0`). **Pengadaan:** dokumen sudah ter-digest generik OTOMATIS saat upload → mulai dengan **`read_ingested_digest`** (memuat dokumen terbaru); identifikasi paket dari isi dokumen, bandingkan dengan nilai `ro` di `temuan.json` → paket tanpa temuan = baru (skenario (e)); dokumen pendukung paket lama = refine (skenario (f)). Jangan menebak — dasarkan pada dokumen yang ada.
...
7. **Setelah digest OK — analisis via CHECKLIST SKILL.** **Skill PBJ kini MODE FULL-AI — TANPA rule/anomali.** RKA-K/L: `run_batch_rka` → **`read_digest`** (index lalu `read_digest(ro=<id>)` per RO). Pengadaan (reviu/audit): dokumen ter-digest generik saat upload → **`read_ingested_digest`** + `read_pdf_page` (PDF/Word/Excel) untuk rincian. Lalu telusuri **TIAP butir Checklist Pemeriksaan** di SKILL → `append_temuan` untuk yang tidak sesuai. Tutup tiap butir: sesuai / tidak sesuai / tidak cukup data.
