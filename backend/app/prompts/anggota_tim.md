...
>   - **⚠ PENGECUALIAN WAJIB untuk RKA-K/L:** RO objek bisa ditambah kapan saja, dan kamu TIDAK bisa tahu ada RO baru tanpa men-digest. Maka **SELALU mulai REFINE RKA dengan `run_batch_rka` (idempoten, ~6 dtk/RO) → `read_digest` (index)**. Index memberi `n_temuan` per RO → RO baru muncul otomatis dengan `n_temuan=0`. Ini menggantikan tebakan "apakah ada RO baru". Lalu jalankan skenario (e).
...
>     - **(e) RO objek baru (khusus RKA-K/L)** — setelah `run_batch_rka` + `read_digest` index (langkah wajib di atas): **analisis HANYA RO dengan `n_temuan=0`** (RO baru); RO dgn `n_temuan>0` = sudah selesai, **JANGAN disentuh/analisis ulang**. Untuk tiap temuan RO baru: `append_temuan` **tanpa `id_temuan`** (id auto) **DAN isi field `ro` = `ro_label`** RO tsb (dari index). Temuan RO lama tidak diubah. Bila SEMUA RO sudah `n_temuan>0` (tak ada RO baru) → perlakukan sebagai skenario (a)-(d) sesuai permintaan auditor.
