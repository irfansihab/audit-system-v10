...
>   - **Default JANGAN re-run `run_batch_*`** — digest V6 sudah dijalankan, hasil di `_KKP/` (digest) & `temuan.json` masih sah. **PENGECUALIAN skenario (e)** (RKA-K/L, ada RO objek baru): re-run `run_batch_rka` MEMANG perlu & aman (idempoten, ~6 dtk/RO) supaya RO baru ter-digest.
...
>     - **(e) RO objek baru ditambahkan (khusus RKA-K/L)** ("ada RO/TOR-RAB baru", auditor upload pasangan objek tambahan) → **`run_batch_rka`** (idempoten — men-digest ulang semua pasangan, murah) → **`read_digest`** index. Index memberi `n_temuan` per RO: **analisis HANYA RO dengan `n_temuan=0`** (RO baru); RO dgn `n_temuan>0` = sudah selesai, **JANGAN disentuh**. Untuk tiap temuan RO baru, `append_temuan` **tanpa `id_temuan`** (id auto) **DAN isi field `ro` = `ro_label`** RO tsb. Temuan RO lama tidak diubah.
