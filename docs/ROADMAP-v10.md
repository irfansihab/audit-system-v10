...
### ✅ Sprint 3 — Hardening dari UAT skill riil (19 Jul 2026, `feat/integral-sprint3`)
UAT langsung auditor (tema pengadaan · RKA-K/L · SPIP) menemukan celah mutu/efisiensi/UX → diperbaiki & disebar lintas-skill:
- **7.5 Analisis inkremental multi-unit** — bedakan **unit BARU** vs **dokumen PENDUKUNG unit lama** (skenario e/f), hindari re-analisis dari nol. Tag **`ro` stabil** per unit + deteksi `n_temuan` (RKA) / bandingkan `ro` di `temuan.json` (non-RKA). Awalnya RKA multi-RO + pengadaan multi-paket; **kini disebar ke SEMUA skill ber-objek jamak** (audit-kinerja multi-program, audit/reviu/evaluasi-umum multi-objek). KKP & UI membedakan unit (heading "Unit Objek"/"Paket"/"RO"). Objek tunggal (LKE/laporan) → `ro` kosong, semua upload = refine. *(commit `db46bce`, `8d3ee48`, `f4859bd`, `6c5044c`, `aef14c7`)*
- **7.6 Penutupan penilaian per aspek (bukan exception-only)** — agen **menilai & mendokumentasikan TIAP butir checklist** (SESUAI/TIDAK_SESUAI/TIDAK_CUKUP_DATA + **dasar**), termasuk yang memadai — bukan hanya berburu diskrepansi termudah. Tool baru `write_penilaian_aspek` → tabel **"Kesimpulan Penilaian per Aspek"** di KKP. **Tervalidasi live** (run #11 pengadaan: **17 butir dinilai** — mutu spesifikasi teknis, identifikasi kebutuhan, 5 elemen justifikasi, konsistensi, kecukupan info). Disebar ke **9 skill KKSA non-LKE** (LKE dikecualikan — sudah tutup per-cell via `write_penilaian_lke`). *(commit `b670de9`, `481631d`)*
- **7.7 Pensiun `digest_pengadaan` → digest generik andal** — parser terstruktur khusus PBJ rapuh pada dokumen riil (mis. nama_pekerjaan tertukar teks Perpres) → diganti **digest generik per dokumen** (`_INGESTED/<jenis>-NN.json`). Ingestion + agen kini terima **PDF · Word `.docx` · Excel `.xlsx`** (`office_extract`; `read_pdf_page` baca docx/xlsx, Excel halaman=sheet). Fix **"failed to fetch"** saat upload (SIGSEGV LiteParse tak thread-safe → `_PARSE_LOCK`). *(commit `d80fec5`, `65ae662`, `9a3d690`)*
- **7.8 Efisiensi & doktrin PKP** — `read_ingested_digest` diperkaya (budget/caps naik) supaya agen **tak sapu-baca PDF** (patokan ≤1–2 `read_pdf_page`/temuan); REFINE tak re-run penuh. **Penilaian/skoring PKP dicabut dari agen** (PKP = **lantai** yang ditetapkan KT/PT, bukan objek nilai; usul langkah cukup di `notes_freetext`). *(commit `2539aad`, `97147de`)*
- **7.9 Tampilan temuan LKE** — rekap skor/predikat (PM vs APIP) + daftar AoI di UI, konsisten format LKE; doktrin incremental LKE (1 objek). *(commit `ae126f2`)*
- **7.10 Pengelolaan Template KP/PKP** — submenu jadi CRUD (PT buat/edit/hapus) + **generate draf dari wiki (AI)**. *(commit `ef24f1b`)*
- **7.11 Chat bebas berbasis wiki (RAG)** — tanya-jawab grounded ke pengetahuan wiki + regulasi-kunci (anti-halusinasi + sumber); cikal bakal chatbot WA/Telegram. *(commit `b09176c`)*
...
- **7.15 Lengkapi office-support jalur RKA `run_batch`** — `d80fec5` sudah meng-upgrade digester daun (`digest_tor.py`/`digest_rab.py` route by suffix), tapi jalur yang DIPAKAI agen (`run_batch_rka` → V6 `run_batch.py` → `read_digest` dari `_KKP/`) masih **filter PDF-only di 3 titik** → TOR/RAB Word/Excel terbuang sebelum sampai digester. **Diperbaiki:** `_stage_rka_inputs` (`_RKA_SUPPORTED_SUFFIXES`), V6 `run_batch.py` (regex `RE_DATED/RE_NODATE` + `_scan_dir` → `SUPPORTED_EXTS`), dan `digest_rab.py` `_office_xlsx_pages` join **TAB** (bukan `" | "`) agar baris komponen SAKTI tetap cocok regex whitespace. **Teruji E2E (2 RO campuran):** RO1 PDF **regresi bersih** (3 komponen, Rp54,45 M) · RO2 `.docx`+`.xlsx` kini terbaca (3 komponen + seluruh identitas TOR, `ro_label`="Sistem Informasi Ekosistem Gim") · `read_digest` menampilkan kedua RO.
